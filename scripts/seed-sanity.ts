/**
 * Seed Sanity CMS with placeholder data.
 *
 * Usage:
 *   1. Create a write token at https://www.sanity.io/manage → API → Tokens
 *   2. Add SANITY_API_WRITE_TOKEN=<token> to .env.local
 *   3. Run: npx tsx scripts/seed-sanity.ts
 *
 * This script is idempotent — documents use fixed _id values so
...
/** Convert plain text to a Sanity blockContent array (single block, normal style) */
...
/** Upload an image from public/ and return an asset reference */
...
const CATEGORIES = [
  ...
  { _id: "cat-mixed", title: { es: "Técnica Mixta", en: "Mixed Media" }, slug: { _type: "slug", current: "mixed-media" }, type: "medium" },
...
const ARTWORKS = [
  {
    ...
    image: "images/artwork/uda-gallery-3.jpg",
  ...
  },
  {
    ...
    categories: ["cat-mixed", "cat-medicine"],
    featured: true,
    order: 5,
  },
];
...
const EXHIBITIONS = [
  ...
  { title: { es: "Primera Exposición", en: "First Exhibition" }, venue: "Galería del Banco del Pacífico", location: "Ecuador", year: 1982, type: "solo" },
];
...
async function seed() {
  ...
  // 3. Upload images & create artworks
  console.log("  Artworks (uploading images)...");
  for (const artwork of ARTWORKS) {
    const { image: imagePath, categories: catIds, ...rest } = artwork;
    console.log(`    ${rest.title.en}...`);
    const imageAsset = await uploadImage(imagePath);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const doc: any = {
      ...rest,
      _type: "artwork",
      images: [{ ...imageAsset, _key: crypto.randomUUID().slice(0, 8) }],
  ...
  // 4. Artist profile
  console.log("  Artist profile (uploading portrait)...");
  const portraitAsset = await uploadImage("images/artist/portrait-bw.jpg");
  const studioPhotos = [];
  for (const photo of ["images/artist/uda-press.jpg", "images/artist/mural-unveiling.webp"]) {
    const asset = await uploadImage(photo);
    studioPhotos.push({ ...asset, _key: crypto.randomUUID().slice(0, 8) });
  }
  ...
  await client.createOrReplace({
    ...
    bio: {
      es: toBlock(ARTIST_TEXT.bio.es),
      en: toBlock(ARTIST_TEXT.bio.en),
    },
    duality: ARTIST_TEXT.duality,
    statement: {
      es: toBlock(ARTIST_TEXT.statement.es),
      en: toBlock(ARTIST_TEXT.statement.en),
    },
    essay: {
      es: toBlock(ARTIST_TEXT.essay.es),
      en: toBlock(ARTIST_TEXT.essay.en),
    },
  ...
  const featuredIds = ARTWORKS.filter((a) => a.featured).map((a) => a._id);
  await client.createOrReplace({
    _id: "homepage-settings",
    _type: "homepage",
    heroPaintings: [
      { _type: "reference", _ref: "artwork-innocence-i", _key: crypto.randomUUID().slice(0, 8) },
    ],
